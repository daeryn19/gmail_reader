...
with open("D:/CODING ATTEMPTS/gmail_reader/creds.yml") as f:
    content = f.read()
...
keywords = ['aliexpress', 'tech expo', "application was sent to", "application was viewed", "unfortunately", "decided not to move forward", "decided to move forward with other candidates", 'noreply', 'no-reply', 'jobs', 'reducere', 'promotie', 'discount', 'sephora', 'carzz', 'bilete', 'greenpeace', "thank you for applying"]
senders = [
...
with MailBox("imap.gmail.com").login(user, password) as mailbox:
    ...
    for msg in mailbox.fetch(limit=500):
